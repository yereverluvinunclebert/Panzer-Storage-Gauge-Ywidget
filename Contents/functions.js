...
var mainWindow, mainWindowwidthDefault, mainWindowheightDefault,
		...
		pin, lock, stopWatchState, mistake, tankHelpShow, facebookChat, storageTimer, secondHand,
		secondShadow, startButton, stopButton, helpButton, tickSwitch, letterBack, updatestorage;
...
function menuitem6OnClick() {
	...
	if (answer === 1) {
		openURL("https://www.deviantart.com/yereverluvinuncleber/art/Panzer-Disc-Storage-Gauge-Yahoo-Widget-737565423");
		return;
	}
}
//=====================
...
function setmenu() {
	mainWindow.onContextMenu = function () {
		...
		items.push(mItem);
	  	...
	  	mItem = new MenuItem();
	  	mItem.title = "Panzer Storage Gauge Help";
	  	mItem.onSelect = function () {
		  	tankHelpShow();
	  	};
		items.push(mItem);
...
function settooltip() {
	...
	if (preferences.tooltipPref.value === "enabled") {
		startButton.tooltip = "Click here to spawn a new widget";
		stopButton.tooltip = "This button is unassigned";
		switchFacesButton.tooltip = "Select next drive here";
		...
		letterBack.tooltip = "Select another drive here by clicking or using mousewheel";
...
tickSwitch.onMouseUp = function () {
	...
	storageTimer.interval = 1; // the interval is temporarily reduced and the update is triggered quickly
	// updatestorage();
